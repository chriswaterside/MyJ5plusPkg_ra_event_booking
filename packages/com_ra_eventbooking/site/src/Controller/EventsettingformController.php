...
namespace Ramblers\Component\Ra_eventbooking\Site\Controller;
...
class EventsettingformController extends FormController {
    ...
    public function display($cachable = false, $urlparams = []) {
        ...
        // Bind item params to form
        $form = $model->getForm($item->getProperties());

        $this->setPathway();
        $this->document->setTitle($item->title);

        $this->item = $item;
        $this->form = $form;  // Pass form to view
    }
    ...
    public function edit($key = NULL, $urlVar = NULL) {
        ...
        // Get the model.
        $model = $this->getModel('Eventsettingform', 'Site');
    ...
    public function save($key = NULL, $urlVar = NULL) {
        ...
        if ($data === false) {
            ...
            $this->app->setUserState('com_ra_eventbooking.edit.eventsetting.data', $jform);

            // Redirect back to the edit screen.
            $id = (int) $this->app->getUserState('com_ra_eventbooking.edit.eventsetting.id');
            $this->setRedirect(Route::_('index.php?option=com_ra_eventbooking&view=eventsettingform&layout=edit&id=' . $id, false));
        ...
        if ($return === false) {
            // Save the data in the session.
            $this->app->setUserState('com_ra_eventbooking.edit.eventsetting.data', $data);
    ...
    public function cancel($key = NULL) {
        ...
        $editId = (int) $this->app->getUserState('com_ra_eventbooking.edit.eventsetting.id');
    ...
    public function remove() {
        ...
        try {
            ...
            if ($return) {
                $model->delete($pk);
                $this->setMessage(Text::_('COM_RA_EVENTBOOKING_ITEM_DELETED_SUCCESSFULLY'));
            } else {
                $this->setMessage(Text::_('COM_RA_EVENTBOOKING_ITEM_DELETED_UNSUCCESSFULLY'), 'warning');
            }
            ...
            $this->setRedirect('index.php?option=com_ra_eventbooking&view=eventsettings');
        }
    }
